...
// mapLetterToNumber maps a letter to a number according to the IBAN specification
func mapLetterToNumber(r rune) string {
	if unicode.IsDigit(r) {
		return string(r)
	}
	return fmt.Sprintf("%d", int(r-'A')+10)
}

// Iban checks if a given iban is valid.
func Iban(iban string) bool {
	...
	mapped := ""
	for _, r := range iban {
		mapped += mapLetterToNumber(r)
	}

	// Parse the string to a big integer
	num := new(big.Int)
	num.SetString(mapped, 10)

	// Compute the remainder of num / 97
	remainder := new(big.Int)
	remainder.Mod(num, big.NewInt(97))

	// The IBAN is valid if and only if remainder == 1
